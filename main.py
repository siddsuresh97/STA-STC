...
def main(run_sta=True, run_stc=True, main_dict_exists = True):
    ...
    #regr = linear_model.LinearRegression()
    #regr.fit(X, cat_spikes)
    #all_stc = stc(video_feats=X, spk_data=cat_spikes, time=time, h =14, w= 14)
    #total_stc = regr.coef_[:, :-2].reshape(h,w)
    #import ipdb;ipdb.set_trace()
    #print("ahhah")
...
'''
 ...
 for idx,i in enumerate(main_dict.keys()):
    ...
    ...:     #regr2.fit(zca_feats, spk_cat_spikes)
    ...:
    ...:     total_stc = regr1.coef_
    ...:     #import ipdb;ipdb.set_trace()
    ...:     plt.subplot(gs[int(idx/2),int(idx%2)])
    ...:     plt.title("STC neuron corresponding to device {}".format(i),fontsiz
    ...: e=1)
    ...:     plt.imshow(regr1.coef_.reshape(14,25),cmap="Reds")
'''
'''
for idx,i in enumerate(main_dict.keys()):
    ...:     cat_feats = main_dict[i]['cat_feats']
    ...
    ...:     zca_feats = zca_whiten(cat_feats)
    ...:     total_sta = zca_feats*spk_cat_spikes
    ...:     per_video_sta = np.array([np.mean(total_sta[np.argwhere(cat_videos==x),:], axis = 0) for x
    ...:  in np.unique(cat_videos)])
    ...:     plt.subplot(gs[int(idx/2),int(idx%2)])
    ...:     plt.title("PER_vid_STA_avg corresponding to device {}".format(i),fontsize=5)
    ...:     plt.imshow(np.mean(per_video_sta, axis = 0 ).reshape(14,25),cmap="Reds")
...
for idx,i in enumerate(main_dict.keys()):
     ...
     ...:     zca_feats = zca_whiten(cat_feats)
     ...:     total_sta = zca_feats*spk_cat_spikes
     ...:     per_video_stc =[np.array(zca_feats[np.argwhere(cat_videos==x),:]) for x in np.unique(cat_
     ...: videos)]
     ...:     feats_per_video = np.array([per_video_stc[i].squeeze() for i in range(len(per_video_stc))
     ...: ])
     ...:     per_video_spk = [np.array(spk_cat_spikes[np.argwhere(cat_videos==x),:]) for x in np.uniqu
     ...: e(cat_videos)]
     ...:     spikes_per_video = np.array([per_video_spk[i].reshape(per_video_spk[i].shape[0],1) for i
     ...: in range(len(per_video_spk))])
     ...:     assert(len(per_video_spk)==len(feats_per_video))
     ...:     stc_per_video = []
     ...:     for i in range(len(feats_per_video)):
     ...:         regr = linear_model.LinearRegression()
     ...
     ...:     plt.subplot(gs[int(idx/2),int(idx%2)])
     ...:     plt.title("PER_vid_STC_avg corresponding to device {}".format(i),fontsize=5)
     ...:     plt.imshow(stc_mean.reshape(14,25),cmap="Reds")
